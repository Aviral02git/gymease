...
const GymCard = ({ id, name, location, monthlyPrice, tier, rating, reviews, image, tags = [] }) => {
  return (
    <Card hoverEffect className="overflow-hidden group flex flex-col h-full !rounded-none border-b-4 border-b-transparent hover:border-b-primary transition-all">
      {/* Image Container */}
      ...
      <div className="p-5 flex-grow flex flex-col bg-surface">
        ...
        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
          <span className="text-xl font-black text-white">{formatINR(monthlyPrice)} <span className="text-xs text-textMuted font-bold uppercase tracking-widest">/mo</span></span>
          <Link to={`/gyms/${id || 1}`} className="text-primary font-black uppercase text-sm tracking-widest hover:text-white transition-colors flex items-center gap-1">
            Explore 
            <svg className="w-4 h-4" justify="center" fill="none" stroke="currentColor" viewBox="0 0 24 24">
